...
/*   Updated: 2020/09/10 13:33:42 by vkurkela         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_types(t_vm *vm, int *args, t_process *proc)
{
	int i;
...
int			validate_encoding(t_vm *vm, int encode_byte, t_process *proc)
{
	...
	return (check_types(vm, arg, proc) && check_op_args(vm, arg, proc->opcode));
}
